...
PLUS_TASK = """
    Your task is to make a **minimal, targeted** change to a given assistant response. The change should be made so that the new response **exhibits** a textual attribute specified below. The change should be **targeted and minimal**: aim to change ONLY the necessary parts of the response needed to make the response exhibit this textual attribute while remaining natural and fluent. This means that you should NOT change ANY other parts of the response that are unnecessary for adding this attribute; for example, if the original response contains mistakes, harmful content, or possibilities for improvements, **ABSOLUTELY DO NOT** modify them unless doing it is necessary for adding the specified attribute.
""".strip()

MINUS_TASK = """
    Your task is to make a **minimal, targeted** change to a given assistant response. The change should be made so that the new response **does not exhibit** a textual attribute specified below. The change should be **targeted and minimal**: aim to change ONLY the necessary parts of the response needed to make the response NOT exhibit this textual attribute while remaining natural and fluent. This means that you should NOT change ANY other parts of the response that are unnecessary for removing this attribute; for example, if the original response contains mistakes, harmful content, or possibilities for improvements, **ABSOLUTELY DO NOT** modify them unless doing it is necessary for removing the specified attribute.
""".strip()
