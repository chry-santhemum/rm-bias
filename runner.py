...
class Runner(ABC):
    ...
    @staticmethod
    def classifier_worker(task_queue, result_queue, rater_config, policy_config):
        """
        Worker process for running GPU-intensive classifier ratings.
        Reconstructs the RewardModel and PolicyModel locally to avoid pickling issues.
        """
        # Lazily construct heavy objects inside the process
        rating_function = RewardModel(
            reward_model_name=rater_config["reward_model_name"],
            batch_size=rater_config.get("batch_size", 32),
        )
        policy_model = PolicyModel(
            model_name=policy_config["model_name"],
            max_tokens=policy_config.get("max_tokens", 1024),
            temperature=policy_config.get("temperature", 0.8),
            max_par=policy_config.get("max_par", 64),
            full_logging=policy_config.get("full_logging", False),
        )

        # Load cached normalization stats if available
        try:
            asyncio.run(normalize(rating_function, policy_model, overwrite=False))
        except Exception:
            pass
        ...
        while True:
            ...
            # Unpack arguments
            seed_state, prompts, n_samples = task_args
    ...
    @staticmethod
    def lm_judge_worker(task_queue, result_queue, rater_config, policy_config):
        """Worker for I/O-intensive LM Judge ratings. Reconstructs models locally."""
        rating_function = LLMJudge(
            judge_model_name=rater_config["judge_model_name"],
            rubric=rater_config["rubric"],
            max_par=rater_config.get("max_par", 256),
            max_tokens=rater_config.get("max_tokens", 4096),
            reasoning=rater_config.get("reasoning", None),
            full_logging=rater_config.get("full_logging", False),
        )
        policy_model = PolicyModel(
            model_name=policy_config["model_name"],
            max_tokens=policy_config.get("max_tokens", 1024),
    ...
    def get_ratings(self, n_samples: int = 1):
        ...
        for seed_state in self.seed_states:
            ...
            if classifier_rater is not None:
                classifier_tasks.append(base_task)
            if lm_rater is not None:
                lm_judge_tasks.append(base_task)
        ...
        # Prepare reconstruction configs
        policy_config = {
            "model_name": self.policy_model.model_name,
            "max_tokens": self.policy_model.max_tokens,
            "temperature": self.policy_model.temperature,
            "max_par": self.policy_model.max_par,
            "full_logging": self.policy_model.full_logging,
        }
        classifier_config = (
            {
                "reward_model_name": classifier_rater.model_name,
                "batch_size": getattr(classifier_rater, "batch_size", 32),
            }
            if classifier_rater is not None
            else None
        )
        lm_judge_config = (
            {
                "judge_model_name": lm_rater.model_name,
                "rubric": getattr(lm_rater, "rubric", ""),
                "max_par": getattr(lm_rater, "max_par", 256),
                "max_tokens": getattr(lm_rater, "max_tokens", 4096),
                "reasoning": getattr(lm_rater, "reasoning", None),
                "full_logging": getattr(lm_rater, "full_logging", False),
            }
            if lm_rater is not None
            else None
        )

        # Start processes conditionally based on tasks/configs
        gpu_process = None
        api_process = None
        if classifier_tasks and classifier_config is not None:
            gpu_process = multiprocessing.Process(
                target=Runner.classifier_worker,
                args=(gpu_queue, result_queue, classifier_config, policy_config),
            )
            gpu_process.start()
        if lm_judge_tasks and lm_judge_config is not None:
            api_process = multiprocessing.Process(
                target=Runner.lm_judge_worker,
                args=(api_queue, result_queue, lm_judge_config, policy_config),
            )
            api_process.start()
