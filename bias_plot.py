...
def plot_bias_results(json_file_path: str, save_path: str|None = None):
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f'{attr}<br>Plus: {score:.3f}' for attr, score in zip(attributes, plus_scores)],
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f'{attr}<br>Minus: {score:.3f}' for attr, score in zip(attributes, minus_scores)],
        hovertemplate='%{text}<extra></extra>'
    ))
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f'{attr}<br>Original: {score:.3f}' for attr, score in zip(attributes, original_scores)],
        hovertemplate='%{text}<extra></extra>'
    ))
