...
class LevelsRunner(Runner):
    ...
    def train(self):
            ...
            for plan in seed_state.history[-1].values():
                level_0_evaluate_tasks.append(
                    self.evaluate_attributes(
                        user_prompts=[plan.meta["user_prompt"]],
        ...
        self.save_attribute_stats(save_dir=self.run_path / "level_0_stats")  # save level 0 info
        ...
        for seed_state_idx, seed_state in enumerate(self.seed_states):
            ...
            level_1_attributes = [next(iter(r)) for r in results[: self.n_pop]]
        ...
        for level_1_result, seed_state in zip(level_1_evaluate_results, self.seed_states):
            for attribute, rollouts in level_1_result.items():
                seed_state.history[-1][attribute].rollouts = rollouts

        self.save_attribute_stats(save_dir=self.run_path / "level_1_stats")  # save level 1 info
