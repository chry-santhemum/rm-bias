...
all_plans = list(runner.seed_states[0].history[-1].keys())
print(f"{len(all_plans)} plans in total.")

# %%
import json

with open("data/exp_test_clusters/all_plans_chatgpt_0.json", "w") as f:
...
for rep in sorted(representatives, key=lambda x: -x["size"])[:10]:
