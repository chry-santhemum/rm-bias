...
async def main(num_prompts: int, n_rollouts: int):
    ...
    for attribute, attribute_rollouts in positive_rewrite_rollouts.items():
        ...
        for prompt, positive_rollouts in attribute_rollouts.items():
            ...
            for positive_rollout, negative_rollout in zip(positive_rollouts, negative_rollouts):
                if positive_rollout is None or negative_rollout is None or positive_rollout.score is None or negative_rollout.score is None:
                    continue
                attribute_diffs.append(positive_rollout.score - negative_rollout.score)
